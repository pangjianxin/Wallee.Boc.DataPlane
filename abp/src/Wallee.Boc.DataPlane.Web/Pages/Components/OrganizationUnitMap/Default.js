...
(function () {
    abp.widgets.OrganizationUnitMap = function ($wrapper) {
        ...
        let switchGeoMap = async function (chart, mapPath, mapName, callback) {
            console.log(echarts.getMap(mapName));
            if (!echarts.getMap(mapName)) {
                var geoJson = await $.get(`${mapPath}${mapName}.json`);
                echarts.registerMap(mapName, { geoJson: geoJson });
                chart.setOption({
                    geo: {
                        show: true,
        ...
        let init = async function (filters) {
            ...
            myChart.setOption(option);
            await switchGeoMap(myChart, ROOT_PATH, initialMapName);
            window.load = echartsResize();
            window.addEventListener("resize", echartsResize);
            myChart.on("click", function (params) {
                console.log(params);
            ...
            var data = await _dashboardService.getConvertedCusOrgUnitDetails(null);
            option && myChart.setOption({
                dataset: {
                    dimension: ['orgName', 'orgidt', 'lng', 'lat', 'value'],
                    source: data.items
